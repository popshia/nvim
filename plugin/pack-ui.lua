...
local function build_content()
   ...
   local help = " [r] refresh  [u] update plugin  [U] update all  [Enter] details  [x] remove plugin  [q] close"
...
local function setup_keymaps()
   ...
   map("x", remove_plugin, "Remove plugin")
   map("]]", function()
      jump(1)
   end, "Next plugin")
   map("[[", function()
...
api.nvim_create_user_command("Pack", function(command)
